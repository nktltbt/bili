...
/**
 * l'activité d'affichage du formulaire pour le remplir
 * @author Faissal
 *
 */
public class RemplirForm extends Activity implements OnClickListener {
	
	TableLayout layout; //pour afficher les champs
	List<String> valeurs = new ArrayList<String>(); // pour recuperer les ids des champs et leur type
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		else //recuperer l'id utilisateur s'il est connecté
		...
		BitmapDrawable backTitre = (BitmapDrawable) this.getResources().getDrawable(R.drawable.titre);
		...
		// - - - - - -  - - Get resolution ecran - - - - - - - - - - 
		 DisplayMetrics dm = new DisplayMetrics();
		    ...
		    int NbrLayout =0; //definir le nombre de colonnes que l'ecran peut afficher (0 càd 1 colonne, 1 càd 2 colonnes ...)
		    width= dm.widthPixels; // largeur ecran
		    
		    Configuration config = getResources().getConfiguration();
		    if (((config.orientation == Configuration.ORIENTATION_PORTRAIT) && screenInches<=6) || width<=400){
		    	//s'il sagit d'un smartphone, dans ce cas on force le mobile d'afficher le formulaire dans une colonne
		    	widthChamp = width;
		    }else{  //si la largeur de l'ecran plus grande, le nombre de colonnes sera auomatique 
		 ...
		 int h=0; //correspond à chaque ligne, chaque valeur de h donne une ligne d'affichage du formulaires
		 int c=0; // le nombre de colonne du champ dans la base
		 
		 
		 TableRow[] row = new TableRow[Config.NBR_ROW]; // chaque row[..] correspond à une ligne du formulaire, max lignes = 50
		 //h=0, c'est la 1ere ligne du formulaire
		 row[0] = new TableRow(this);
		 
			Cursor curelement = elementdb.getElements(idF); //recuperer tout les champs du formulaire
			if(curelement.getCount()>0){ //s'il y a pas de champ
				...
				int pos_x;     // sa position dans le grid (pos_x --> colonne)
				 ...
				 if(type.equals("text")){ //s'il sagit d'un texte
					 ...
					 if(curParam.getCount()>0){
						 curParam.moveToFirst();
							 while (curParam.isAfterLast() == false) {
								 ...
								 if(curParam.getString(1).equals("taille_police"))        taille_police = curParam.getString(2);
								 else if(curParam.getString(1).equals("couleur"))         couleur_police =curParam.getString(2);
								 else if(curParam.getString(1).equals("titre"))           titre =curParam.getString(2);
								 else if(curParam.getString(1).equals("sousligne"))       sousligne = curParam.getString(2);
								 else if(curParam.getString(1).equals("gras"))            gras =curParam.getString(2);
								 else if(curParam.getString(1).equals("italique"))        italique = curParam.getString(2);
								 else if(curParam.getString(1).equals("dimension"))       dimension = curParam.getString(2);
								 else if(curParam.getString(1).equals("persistent"))      persistent = curParam.getString(2);
					 ...
					 if(c==pos_x && c<=NbrLayout) { 
						 ...
						 row[h].addView(addEditText(idElement,type, NbrLayout, titre, taille_police, couleur_police, sousligne, gras, italique, dimension, persistent));
						 c++;
					 }else {    
						 //si l'ecran ne peut pas afficher le champ dans leur position 
			    ...
			    else if(type.equals("checkbox")){  //s'il sagit d'un checkbox
				 String titre= null;
				...
				//recuperer les parametre du champ
				 ...
				 if(curParam.getCount()>0){
					 curParam.moveToFirst();
						 while (curParam.isAfterLast() == false) {
							 ...
							 if(curParam.getString(1).equals("titre"))        titre = curParam.getString(2);
							 else if(curParam.getString(1).equals("libelles"))         libelles =curParam.getString(2);
							 else if(curParam.getString(1).equals("orientation"))       orientation = curParam.getString(2);
				 ...
				 if(c==pos_x && c<=NbrLayout) {
					//si l'ecran peut afficher le champ dans leur position 
					 row[h].addView(addCheckBox(idElement, type, NbrLayout, titre, libelles, orientation));
					 c++;
				 }else {
					//si l'ecran ne peut pas afficher le champ dans leur position 
						  layout.addView(row[h]); //ajouter la ligne actuel
				...
				//recuperer les parametre du champ
				 ...
				 if(c==pos_x && c<=NbrLayout) {
					//si l'ecran peut afficher le champ dans leur position 
					 row[h].addView(addRadio(idElement, type, NbrLayout, titre, libelles, orientation));
					 c++;
				 }else {
					//si l'ecran ne peut pas afficher le champ dans leur position 
						  layout.addView(row[h]);//ajouter la ligne actuel
			 ...
			 }else if(type.equals("combobox")){   //s'il sagit d'un combobox
				 String titre= null;
				 String libelles = null;
				 String orientation = null;
				//recuperer les parametre du champ
				 ...
				 if(c==pos_x && c<=NbrLayout) {
					//si l'ecran peut afficher le champ dans leur position 
					 row[h].addView(addComboBox(idElement, type, NbrLayout, titre, libelles, orientation));
					 c++;
				 }else {
					//si l'ecran ne peut pas afficher le champ dans leur position 
						 layout.addView(row[h]);//ajouter la ligne actuel
					//preparation d'une nouvelle ligne
					 ...
					 c++;
							...
							//recuperer les parametre du champ
							 ...
							 if(c==pos_x && c<=NbrLayout) {
								//si l'ecran peut afficher le champ dans leur position 
								 row[h].addView(addDate(idElement, type, NbrLayout, titre));
								 c++;
							 }else {
								//si l'ecran ne peut pas afficher le champ dans leur position 
									 layout.addView(row[h]);//ajouter la ligne actuel
				...
				//recuperer les parametre du champ
				 ...
				 if(c==pos_x && c<=NbrLayout) {
					//si l'ecran peut afficher le champ dans leur position
					 row[h].addView(addHeure(idElement, type, NbrLayout, titre));
					 c++;
				 }else {
					//si l'ecran ne peut pas afficher le champ dans leur position 
						layout.addView(row[h]);//ajouter la ligne actuel
				...
				//recuperer les parametre du champ
				 ...
				 if(c==pos_x && c<=NbrLayout) {
					//si l'ecran peut afficher le champ dans leur position
					  row[h].addView(addEditNumber(idElement, type, NbrLayout, titre, taille_police, couleur_police, sousligne, gras, italique, dimension, persistent));
					  c++;
				 }else {
					//si l'ecran ne peut pas afficher le champ dans leur position 
					 layout.addView(row[h]);  //ajouter la ligne actuel
...
//	 }
	 ...
	 * @return
	 */
	private View addHeure(int idElement, String type,int nbrlayout, String titre) {
		LinearLayout l = new LinearLayout(this);
		
		TextView view = new TextView(this);
		if(titre==null) titre="heure";
		view.setText(titre);
...
//		view.setBackgroundDrawable(backTitre);
	 ...
	 * @return
	 */
	private View addRadio(int idElement, String type, int nbrlayout, String titre, String libelles, String orientation) {
		LinearLayout l = new LinearLayout(this);
		
		TextView view = new TextView(this);
		if(titre==null) titre="selectionner";
		view.setText(titre);
		view.setWidth((int) (widthChamp*0.95));
		view.setTextSize(16);
		view.setTextColor(Color.parseColor("#0F74B3"));
...
//		String[] libelle = libelles.split("/");
	 ...
	 */
	@SuppressWarnings("deprecation")
	private View addCheckBox(int idElement, String type, int nbrlayout, String titre, String libelles, String orientation) {
	LinearLayout l = new LinearLayout(this);
...
//		String[] libelle = libelles.split("/");
	 ...
	 * @return
	 */
	private View addComboBox(int idElement, String type, int nbrlayout, String titre, String libelles, String orientation) {
		LinearLayout l = new LinearLayout(this);
...
//			libel = new StringTokenizer(libelles, "/"); 	
			libel = new StringTokenizer(libelles, "\\*_\\*"); 
			 ...
			 while (libel.hasMoreElements()) {  
				 list.add(String.valueOf(libel.nextElement()));
			 }
	 ...
	 * @return
	 */
	private View addEditText(int idElement, String type, int nbrlayout, String titre, String taille_police, String couleur_police, String sousligne, String gras, String italique, String dimension, String persistent) {
		
		LinearLayout l = new LinearLayout(this);
		
		TextView view = new TextView(this);
		if(titre==null) titre="texte";
		view.setText(titre);
		view.setWidth((int) (widthChamp*0.95));
		view.setTextSize(16);
		view.setTextColor(Color.parseColor("#0F74B3"));
...
//		view.setBackgroundDrawable(backTitre);
	 ...
	 * @return
	 */
private View addEditNumber(int idElement, String type, int nbrlayout, String titre, String taille_police, String couleur_police, String sousligne, String gras, String italique, String dimension, String persistent) {
		
		LinearLayout l = new LinearLayout(this);
		
		TextView view = new TextView(this);
		if(titre==null) titre="chiffre";
		view.setText(titre);
		view.setWidth((int) (widthChamp*0.95));
		view.setTextSize(16);
		view.setTextColor(Color.parseColor("#0F74B3"));
...
//		view.setBackgroundDrawable(backTitre);
	...
	public void onClick(View v) {
		...
		String id=null;
		String valeur;
		  enregistrer.putExtra("version", 0);
		...
		while(i.hasNext()){
		  ...
		  //traitement du champs par type
			 if(type.equals("text"))  {
				 valeur=(((EditText)findViewById(Integer.valueOf(id))).getText().toString());
				 if(valeur.equals("")) valeur=" ";
				 ...
				 enregistrer.putExtra("element"+e, valeur+"#0_0#"+id);
				  ...
				  RadioGroup rg = ((RadioGroup)findViewById(Integer.valueOf(id)));
				  RadioButton radio_checked = (RadioButton)findViewById(rg.getCheckedRadioButtonId());
				  if(radio_checked!=null){
					  valeur=(radio_checked.getText().toString());
					  if(!valeur.equals("")) valeur = valeur+"#0_0#"+id;
					  else valeur ="X"+"#0_0#"+id;;
					  enregistrer.putExtra("element"+e, valeur);
				  }else enregistrer.putExtra("element"+e, " "+"#0_0#"+id);
			  }
			 else if(type.equals("checkbox")){
				 String rs="";
				 for(int j=0; j<Integer.valueOf(s[2]); j++){
					 if(((CheckBox)findViewById(Integer.valueOf(id+""+j))).isChecked()){
						 valeur = (((CheckBox)findViewById(Integer.valueOf(id+""+j))).getText().toString());
						 
						 if(valeur.equals("")) valeur ="X";
				 ...
				 if(rs!="") enregistrer.putExtra("element"+e, rs+"#0_0#"+id);
				 else enregistrer.putExtra("element"+e, " "+"#0_0#"+id);
			  }
			 else if(type.equals("combobox")) {
				 if(((Spinner)findViewById(Integer.valueOf(id))).getSelectedItem()==null) valeur=" "+"#0_0#"+id;
				 else valeur = (((Spinner)findViewById(Integer.valueOf(id))).getSelectedItem().toString()+"#0_0#"+id);
				 
				 enregistrer.putExtra("element"+e, valeur);
			 }
			 else if(type.equals("date")) {
				 String date = ((EditText)findViewById(Integer.valueOf(id))).getText().toString();
				 if(date.equals("")) date=" ";
				 enregistrer.putExtra("element"+e, date+"#0_0#"+id);
			 }
			 else if(type.equals("time")) {
				 String h = ((EditText)findViewById(Integer.valueOf(id))).getText().toString();
				 if(h.equals("")) h=" ";
		...
		while(ds.hasNext()){
		  final int dateP = (Integer) ds.next();
			if(v.getId()==dateP){
				String d = ((EditText)findViewById(dateP)).getText().toString();
				String[] date = d.split("-");
				...
				DatePickerDialog datedialog = new DatePickerDialog(this,new OnDateSetListener() {
				    ...
				    public void onDateSet(DatePicker datePicker, int i, int j, int k) {
				    	String d = " ";
				    	if(j<9) d=i+"-"+0+""+(j+1); else d=i+"-"+(j+1);
				    	if(k<10) d+="-"+0+""+k; else d+="-"+k;
		...
		while(hs.hasNext()){
		  ...
		  final int heureP = (Integer) hs.next();
			if(v.getId()==heureP){
				String h = ((EditText)findViewById(heureP)).getText().toString();
				String[] date = h.split(":");
				int hr = Integer.valueOf(date[0]);
				int m = Integer.valueOf(date[1]);
				TimePickerDialog datedialog = new TimePickerDialog(this,new OnTimeSetListener() {
