...
import com.applifit.bilifit.MainActivity;
...
/**
 * classe qui synchronise les formulaires après l'authentification d'utilisateur ou le clique sur le bouton synchroniser de la page du profile
 * @author Faissal
 *
 */
...
//		    -------------------  Progress bar -------------------------
			...
			if(cd.isConnectingToInternet()){ //teste l'existance d'internet
				...
				int indice;
				if(list_form.size()!=0)  indice = (100/list_form.size());
				else indice = 100;
				...
				if(cree!=0){
					...
					Notification mNotification = new 
					 Notification(R.drawable.ic_form_notif,MyText,System.currentTimeMillis());
					...
					if (progressBarStatus >= 100) {
						...
						Intent bindIntent = new Intent(Install.this, MyService.class);
						startService(bindIntent);
						
						Bundle extra = getIntent().getExtras();								
						Intent intent = new Intent(Install.this, MainActivity.class);
						intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
						if(extra!=null) intent.putExtra("current", extra.getInt("to"));
