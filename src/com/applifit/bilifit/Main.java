...
import com.applifit.bilifit.DataBase.Install;
import com.applifit.bilifit.DataBase.Syncronisation;
import com.applifit.bilifit.compte.DbUtilisateur;
...
public class Main extends Activity implements OnClickListener {
	...
	//Ajouter les diffÈrents layouts contenant nos vues(composants)
	...
	//la fonction oncreate qui se lance automatiquement lors de lancement de l'activitÈ (Main.java)
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		...
		//detecte l'utilisateur connectÈ
...
//		textPasse.setPadding(60, 32, 0, 0);
		...
		//attribuer les vues ‡ ses layouts
...
//		getMenuInflater().inflate(R.menu.main, menu);
	...
	/**
	 * action du bouton login en envoyant les donnÈes ‡ l'activitÈ "VerifLogin" pour les verifier
	 */
	@Override
	public void onClick(View arg0) {
		...
		if(cd.isConnectingToInternet()){ // teste connexion internet
			//recuperer les infos d'utilisateur enregistrÈes dans la base distant
			...
			if(userOn.getId()!=0 && userOn.getMail()!=null){ //verifier l'existance de l'utilisateur
				if(userOn.getPasse().equals(passe) && (userOn.getProfile().equals("CU") || userOn.getProfile().equals("CA"))){ //verifier ses infos 
						if(userOn.getDevice().equals(IdDevice)){ // si l'utilisateur a ÈtÈ deja connectÈ par le meme mobile
							Toast.makeText(this," vous êtes conneté", Toast.LENGTH_LONG).show();
							count = 2;
						}else if(userOn.getDevice().equals("null")){ // si l'utilisateur n'a jamais connectÈ par un mobile
							Toast.makeText(this," vous êtes conneté", Toast.LENGTH_LONG).show();
							count = 2;
							sync.modifDevice(userOn.getId(), IdDevice);
						}else { // si l'utilisateur tente de se connecter par un autre mobile
							Toast.makeText(this," connecté depuis un autre mobile", Toast.LENGTH_LONG).show();
						}
...
}else  Toast.makeText(this, "Vous ne possédez pas de connexion internet !!", Toast.LENGTH_LONG).show();
